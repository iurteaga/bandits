...
######## Class definition ########
class VariationalBanditSampling(BanditSampling):
    ...
    def update_reward_posterior_variational_resp(self, t):
        ...
        if self.reward_prior['type'] == 'linear_gaussian_mixture' and self.reward_prior['dist'] == 'NIG':
            ...
            rho=( -0.5*(np.log(self.reward_posterior['beta'])-special.digamma(self.reward_posterior['alpha']))[:,:,None]
                - 0.5*(np.einsum('it,akij,jt->akt', self.context[:,:t+1], self.reward_posterior['Sigma'], self.context[:,:t+1])+np.power(self.rewards[:,:t+1].sum(axis=0)-np.einsum('it,aki->akt', self.context[:,:t+1], self.reward_posterior['theta']),2)*self.reward_posterior['alpha'][:,:,None]/self.reward_posterior['beta'][:,:,None])
    ...
    def update_reward_posterior_variational_lowerbound(self,t):
        ...
        tmp=( (np.log(2*np.pi)+np.log(self.reward_posterior['beta'])-special.digamma(self.reward_posterior['alpha']))[:,:,None] 
                ...
                + np.power(self.rewards[:,:t+1].sum(axis=0)-np.einsum('it,aki->akt', self.context[:,:t+1], self.reward_posterior['theta']),2) * self.reward_posterior['alpha'][:,:,None]/self.reward_posterior['beta'][:,:,None] )
