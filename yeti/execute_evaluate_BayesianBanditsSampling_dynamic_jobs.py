...
def main(A, t_max, R, exec_type, theta_min, theta_max, theta_diff):
    print('Executing MC Bayesian {}-armed bandit with TS and new policy with {} time-instants and {} realizations'.format(A, t_max, R))
    ...
    with open('bayesian_bandits_sampling_job.sh') as template_script:
        ...
        for theta in combinations(np.arange(theta_min,theta_max,theta_diff),A):
            ...
            new_job_name='evaluate_BayesianBanditsSampling_dynamic_job_A_{}_tmax_{}_R_{}_exectype_{}_theta_{}'.format(A,t_max,R,exec_type,theta_str)
            with open(new_job_name+'.sh', 'w') as new_job_script:
                ...
                new_script_data=re.sub('python_job_to_run','python $PBS_O_INITDIR/../scripts/evaluate_BayesianBanditsSampling_dynamic.py -A {} -t_max {} -R {} -exec_type {} -theta {}'.format(A,t_max,R,exec_type,theta_str.replace('_', ' ')),new_script_data)
...
if __name__ == '__main__':
    # Input parser
    parser = argparse.ArgumentParser(description='Evaluate Bayesian Bandits with TS and new policy with as different jobs')
