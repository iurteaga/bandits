...
# Main code
def main(M, K, t_max, R, theta_min, theta_max, theta_diff):
    ...
    with open('bayesian_bandits_sampling_job.sh') as template_script:
        ...
        for theta in combinations(np.arange(theta_min,theta_max,theta_diff),K):
            ...
            new_job_name='bayesian_bandits_sampling_dynamic_job_K_{}_tmax_{}_R_{}_M_{}_theta_{}.sh'.format(K,t_max,R,M,theta_str)
            with open(new_job_name, 'w') as new_job_script:
                ...
                new_script_data=re.sub('python_job_to_run','python $PBS_O_INITDIR/../scripts/evaluate_bayesian_bandits_sampling_dynamic.py -M {} -K {} -t_max {} -R {} -theta {}'.format(M,K,t_max,R,theta_str.replace('_', ' ')),new_script_data)
...
if __name__ == '__main__':
    ...
    # Call main function
    main(args.M, args.K, args.t_max, args.R, args.theta_min, args.theta_max, args.theta_diff)
