...
# Main code
def main(A, t_max, R, theta_min, theta_max, theta_diff):
    print('Executing jobs for bayesian {}-armed bayesian bandit for {} time-instants and {} realizations'.format(A, t_max, R))
    ...
    with open('bayesian_bandits_sampling_job.sh') as template_script:
        ...
        for theta in combinations(np.arange(theta_min,theta_max,theta_diff),A):
            ...
            new_job_name='bayesian_bandits_sampling_TS_job_A_{}_tmax_{}_R_{}_theta_{}.sh'.format(A,t_max,R,theta_str)
            with open(new_job_name, 'w') as new_job_script:
                ...
                new_script_data=re.sub('python_job_to_run','python $PBS_O_INITDIR/../scripts/evaluate_bayesian_bandits_sampling_TS.py -A {} -t_max {} -R {} -theta {}'.format(A,t_max,R,theta_str.replace('_', ' ')),new_script_data)
...
if __name__ == '__main__':
    ...
    # Call main function
    main(args.A, args.t_max, args.R, args.theta_min, args.theta_max, args.theta_diff)
