...
def main(A, t_max, M, N_max, R, exec_type, theta, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual logistic bandit with TS, tGaussian arm sampling policy and BUCB for {} time-instants and {} realizations'.format(A, M, t_max, R))
    ...
    for m in Ms:
        ...
        bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior, mc_quantile_empirical))
        bandits_labels.append('MC BUCB, alpha=1/t, empirical M_theta={}'.format(mc_reward_prior['M']))
        # MC based sampling
        mc_quantile_sampling={'MC_alpha':'alpha', 'alpha':alpha, 'type':'sampling', 'n_samples':m}
        bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior, mc_quantile_sampling))
        bandits_labels.append('MC BUCB, alpha=1/t, sampling M_theta={}'.format(mc_reward_prior['M']))
    ...
    bandits_colors=[colors.cnames['purple'], colors.cnames['chocolate'], colors.cnames['pink'], colors.cnames['skyblue'], colors.cnames['palegreen'], colors.cnames['yellow'], colors.cnames['cyan'], colors.cnames['lime'], colors.cnames['orange'], colors.cnames['blue'],  colors.cnames['green'], colors.cnames['red']]
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_logistic_MCBandit_M.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 
    parser = argparse.ArgumentParser(description='Evaluate Contextual Logistic Bandits MC posterior: TS, truncated Gaussian arm sampling and BUCB policies.')
