...
def main(A, K, t_max, R, exec_type, pi, theta, sigma, d_context, type_context, prior_K):
    ...
    dir_string='../results/{}/A={}/t_max={}/R={}/d_context={}/type_context={}/pi={}/theta={}/sigma={}/prior_K={}'.format(os.path.basename(__file__).split('.')[0], A, t_max, R, d_context, type_context, str.replace(str.strip(np.array_str(pi.flatten()),' []'), '  ', '_'), str.replace(str.strip(np.array_str(theta.flatten()),' []'), '  ', '_'), str.replace(str.strip(np.array_str(sigma.flatten()),' []'), '  ', '_'), str.replace(str.strip(np.array_str(prior_K.flatten()),' []'), '  ', '_'))
    ...
    # Mixture expectation computation
    mixture_expectations=['z_sampling', 'pi_sampling', 'pi_expected']
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_ContextualLinearGaussianMixture_BanditSampling_VTS.py -A 2 -K 2 -t_max 10 -R 2 -exec_type sequential -d_context 2 -type_context randn -pi 0.5 0.5 0.1 0.9 -theta 1 1 0 0 -1 -1 2 2 -sigma 1 1 1 1 -prior_K 2
    ...
    # Call main function
    main(args.A, args.K, args.t_max, args.R, args.exec_type, pi, theta, sigma, args.d_context, args.type_context, np.array(args.prior_K))
