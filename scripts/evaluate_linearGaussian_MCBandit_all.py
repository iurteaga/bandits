...
def main(A, t_max, M, N_max, R, exec_type, theta, sigma, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual Linear Gaussian bandit with TS, truncated Gaussian arm sampling policy with {} MC samples and BUCB for {} time-instants and {} realizations'.format(A, M, t_max, R))
    ...
    bandits_labels.append('tGaussian (known $\sigma_y^2$): log10(1/Pfa), M_a={}'.format(invPfaSampling['M']))
    # Unknown sigma
    bandits.append(BayesianBanditSampling(A, reward_function, reward_prior_unknown, invPfaSampling))
    bandits_labels.append('tGaussian (unknown $\sigma_y^2$): log10(1/Pfa), M_a={}'.format(invPfaSampling['M']))
    
    # Monte-Carlo based
    # Known sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_known, invPfaSampling))
    bandits_labels.append('MC tGaussian (known $\sigma_y^2$): log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_known['M']))
    # Unknown sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_unknown, invPfaSampling))
    bandits_labels.append('MC tGaussian (unknown $\sigma_y^2$): log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_unknown['M']))
    '''
    ...
    bandits_labels.append('MC-BUCB (known $\sigma_y^2$), M_theta={}, alpha=1/t'.format(mc_reward_prior_known['M']))
    # Unknown sigma
    bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior_unknown, quantileInfo))
    bandits_labels.append('MC-BUCB (unknown $\sigma_y^2$), M_theta={}, alpha=1/t'.format(mc_reward_prior_unknown['M']))
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_linearGaussian_MCBandit_all.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 -sigma 1 1
