...
def main(A, t_max, M, N_max, R, exec_type, theta, sigma, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual Linear Gaussian bandit with TS and sampling policies with {} MC samples for {} time-instants and {} realizations'.format(A, M, t_max, R))
...
if __name__ == '__main__':
    # Input parser
    # Example:  python3 -m pdb evaluate_ContextualLinearGaussianBandits.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 -sigma 1 1
    parser = argparse.ArgumentParser(description='Evaluate Contextual Linear Gaussian Bandits: TS and truncated Gaussian arm sampling policy.')
