...
def main(A, t_max, M, N_max, R, exec_type, theta):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Bernoulli bandit with optimal, TS and all arm sampling policies with {} MC samples for {} time-instants and {} realizations'.format(A, M, t_max, R))
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_Bernoulli_BanditSampling_all.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type sequential -theta 0.2 0.5
    parser = argparse.ArgumentParser(description='Evaluate Bernoulli Bandits: optimal, TS and all arm sampling policy approaches.')
