...
def main(A, t_max, M, R, exec_type, theta, sigma, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual Linear Gaussian bandit with MC-BUCB policies for {} time-instants and {} realizations'.format(A, M, t_max, R))
    ...
    bandits_labels.append('B-UCB (known $\sigma_y^2$), alpha=1/t')
    # Unknown sigma
    bandits.append(BayesianBanditQuantiles(A, reward_function, reward_prior_unknown, quantileInfo))
    bandits_labels.append('B-UCB (unknown $\sigma_y^2$), alpha=1/t')
    ...
    bandits_labels.append('MC-BUCB (known $\sigma_y^2$), M_theta={}, alpha=1/t'.format(M))
    # Unknown sigma
    bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior_unknown, quantileInfo))
    bandits_labels.append('MC-BUCB (unknown $\sigma_y^2$), M_theta={}, alpha=1/t'.format(M))
    
    # MC Empirical, alpha_plus_mcsigma 
    quantileInfo={'MC_alpha':'alpha_plus_mcsigma', 'alpha':alpha, 'type':'empirical'}    
    # Known sigma
    bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior_known, quantileInfo))
    bandits_labels.append('MC-BUCB (known $\sigma_y^2$), M_theta={}, alpha=1/t+mcsigma'.format(M))
    # Unknown sigma
    bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior_unknown, quantileInfo))
    bandits_labels.append('MC-BUCB (unknown $\sigma_y^2$), M_theta={}, alpha=1/t+mcsigma'.format(M))
    
    # MC Empirical, alpha_times_mcsigma 
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_linearGaussian_MCBanditQuantiles_alpha.py -A 2 -t_max 10 -M 50 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 -sigma 1 1
    parser = argparse.ArgumentParser(description='Evaluate Contextual Linear Gaussian BanditQuantiles MC posterior: MC-BUCB policies with alpha')
