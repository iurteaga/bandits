...
def main(A, t_max, M, R, exec_type, theta, sigma, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual Linear Gaussian bandit with Bayesian UCB and MC-BUCB policies for {} time-instants and {} realizations'.format(A, M, t_max, R))
    ...
    for m in Ms:
        ...
        this_mc_reward_prior_known=mc_reward_prior_known.copy()
        this_mc_reward_prior_unknown=mc_reward_prior_unknown.copy()
        this_mc_reward_prior_known['M']=m
        this_mc_reward_prior_unknown['M']=m        

        # Known sigma
        bandits.append(MCBanditQuantiles(A, reward_function, this_mc_reward_prior_known, mc_quantile_empirical))
        bandits_labels.append('MC-BUCB (known $\sigma_y^2$), empirical M={}, alpha=1/t'.format(m))

        # Unknown sigma
        bandits.append(MCBanditQuantiles(A, reward_function, this_mc_reward_prior_unknown, mc_quantile_empirical))
        bandits_labels.append('MC-BUCB (unknown $\sigma_y^2$), empirical M={}, alpha=1/t'.format(m))
        
        # MC based sampling
        mc_quantile_sampling={'MC_alpha':'alpha', 'alpha':alpha, 'type':'sampling', 'n_samples':m}
        
        # Known sigma
        bandits.append(MCBanditQuantiles(A, reward_function, this_mc_reward_prior_known, mc_quantile_sampling))
        bandits_labels.append('MC-BUCB (known $\sigma_y^2$), sampling M={}, alpha=1/t'.format(m))

        # Unknown sigma
        bandits.append(MCBanditQuantiles(A, reward_function, this_mc_reward_prior_unknown, mc_quantile_sampling))
        bandits_labels.append('MC-BUCB (unknown $\sigma_y^2$), sampling M={}, alpha=1/t'.format(m))
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_linearGaussian_MCBanditQuantiles_M.py -A 2 -t_max 10 -M 50 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 -sigma 1 1
    parser = argparse.ArgumentParser(description='Evaluate Contextual Linear Gaussian BanditQuantiles MC posterior: Bayesian UCB and MC-BUCB policies')
