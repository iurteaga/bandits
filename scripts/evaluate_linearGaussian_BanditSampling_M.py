...
def main(A, t_max, M, N_max, R, exec_type, theta, sigma, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual Linear Gaussian bandit with optimal, TS and truncated Gaussian arm sampling policy with {} MC samples for {} time-instants and {} realizations'.format(A, M, t_max, R))

    # Directory configuration
    dir_string='../results/{}/A={}/t_max={}/R={}/M={}/N_max={}/d_context={}/type_context={}/theta={}/sigma={}'.format(os.path.basename(__file__).split('.')[0], A, t_max, R, 0, N_max, d_context, type_context, str.replace(str.strip(np.array_str(theta.flatten()),' []'), '  ', '_'), str.replace(str.strip(np.array_str(sigma.flatten()),' []'), '  ', '_'))
    ...
    #for m in M:
    for m in np.array([100, 500, 1000, 2000]):
        # Truncated Gaussian with log10(1/Pfa), MC over arms with provided M and N_max    
        invPfaSampling={'sampling_type':'infPfa', 'Pfa':'tGaussian', 'f(1/Pfa)':np.log10, 'MC_type':'MC_arms', 'N_max':N_max, 'M':m}
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_linearGaussian_BanditSampling_M.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 -sigma 1 1
    parser = argparse.ArgumentParser(description='Evaluate Contextual Linear Gaussian Bandits: : TS and truncated Gaussian arm sampling policy with different MC accuracies.')
    ...
    parser.add_argument('-M', nargs='+', type=int, default=1000, help='Number of samples for the MC integration')
    ...
    # Call main function
    main(args.A, args.t_max, np.array(args.M), args.N_max, args.R, args.exec_type, theta, sigma, args.d_context, args.type_context)
