...
def main(A, t_max, M, N_max, R, exec_type, theta, sigma, d_context, type_context):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Contextual Linear Gaussian bandit with TS and arm sampling policies with {} MC samples for {} time-instants and {} realizations'.format(A, M, t_max, R))
    ...
    mc_reward_prior_known={'dist':'NIG', 'theta':np.ones((A,d_context)), 'Sigma':Sigmas, 'sampling':'density', 'sampling_sigma':min_sampling_sigma, 'M':M}
    mc_reward_prior_unknown={'dist':'NIG', 'theta':np.ones((A,d_context)), 'Sigma':Sigmas, 'alpha':np.ones((A,1)), 'beta':np.ones((A,1)), 'sampling':'density', 'sampling_sigma':min_sampling_sigma, 'M':M}
    ############################### BANDITS  ###############################    
    ...
    # Monte-Carlo based
    # Known sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_known, thompsonSampling))
    bandits_labels.append('MC Thompson Sampling (known $\sigma_y^2$), M_theta={}'.format(mc_reward_prior_known['M']))
    # Unknown sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_unknown, thompsonSampling))
    bandits_labels.append('MC Thompson Sampling (unknown $\sigma_y^2$), M_theta={}'.format(mc_reward_prior_unknown['M']))
    ...
    # Monte-Carlo based
    # Known sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_known, invPfaSampling))
    bandits_labels.append('MC tGaussian log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_known['M']))
    # Unknown sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_known, invPfaSampling))
    bandits_labels.append('MC tGaussian log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_unknown['M']))
    ...
    # Known sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_known, invPfaSampling))
    bandits_labels.append('MC Chebyshev log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_known['M']))
    # Unknown sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_unknown, invPfaSampling))
    bandits_labels.append('MC Chebyshev log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_unknown['M']))
    ...
    # Known sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_known, invPfaSampling))
    bandits_labels.append('MC Markov log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_known['M']))
    # Unknown sigma
    bandits.append(MCBanditSampling(A, reward_function, mc_reward_prior_unknown, invPfaSampling))
    bandits_labels.append('MC Markov log10(1/Pfa), M_a={}, M_theta={}'.format(invPfaSampling['M'], mc_reward_prior_unknown['M']))
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_linearGaussian_MCBanditSampling_all.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type batch -d_context 2 -type_context randn -theta 1 1 -1 -1 -sigma 1 1
    parser = argparse.ArgumentParser(description='Evaluate Contextual Linear Gaussian Bandits and MC: TS and all arm sampling policies.')
