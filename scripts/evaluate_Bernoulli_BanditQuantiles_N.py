...
def main(A, t_max, M, R, exec_type, theta):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Bernoulli bandit with Bayesian UCB and MC-BUCB policy sampling for {} time-instants and {} realizations'.format(A, M, t_max, R))
    ...
    mc_reward_prior={'dist': stats.beta, 'alpha': np.ones((A,1)), 'beta': np.ones((A,1)), 'sampling':'density', 'sampling_sigma':0.01, 'M':M} 
    ...
    mc_quantileInfo={'MC_alpha':'alpha', 'alpha':alpha, 'type':'empirical'}
    ...
    # MC Empirical
    bandits.append(MCBanditQuantiles(A, reward_function, mc_reward_prior, mc_quantileInfo))
    bandits_labels.append('MC-BUCB, M={}, alpha=1/t'.format(M))
    ...
    for n in Ns:
        ...
        bandits.append(BayesianBanditQuantiles(A, reward_function, reward_prior, quantileInfo))
        bandits_labels.append('Sampling BUCB N={}, alpha=1/t'.format(n))
        ...
        bandits_labels.append('Sampling MC-BUCB, M={}, N={}, alpha=1/t'.format(M,n))
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_Bernoulli_BanditQuantiles_N.py -A 2 -t_max 10 -M 50 -R 2 -exec_type sequential -theta 0.2 0.5
    parser = argparse.ArgumentParser(description='Evaluate Bernoulli BanditQuantiles: Bayesian UCB and MC-BUCB policies with quantile sampling')
