...
def main(A, t_max, M, N_max, R, exec_type, theta):
    ...
    ############################### MAIN CONFIG  ############################### 
    print('{}-armed Bernoulli bandit with TS and truncated Gaussian arm sampling policy with different MC samples for {} time-instants and {} realizations'.format(A, M, t_max, R))

    # Directory configuration
    dir_string='../results/{}/A={}/t_max={}/R={}/M={}/N_max={}/theta={}'.format(os.path.basename(__file__).split('.')[0], A, t_max, R, 0, N_max, str.replace(str.strip(np.array_str(theta.flatten()),' []'), '  ', '_'))
...
if __name__ == '__main__':
    # Input parser
    # Example: python3 -m pdb evaluate_Bernoulli_BanditSampling_M.py -A 2 -t_max 10 -M 50 -N_max 20 -R 2 -exec_type sequential -theta 0.2 0.5
    parser = argparse.ArgumentParser(description='Evaluate Bernoulli Bandits: TS and truncated Gaussian arm sampling policy with different MC accuracies.')
    ...
    # Call main function
    main(args.A, args.t_max, np.array(args.M), args.N_max, args.R, args.exec_type, theta)
